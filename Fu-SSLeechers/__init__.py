...
class Plugin(BasePlugin):
    ...
    def check_user(self, user, num_files, num_folders):
        ...
        if not is_user_accepted:
            ...
            if not self.settings["suppress_all_messages"]:
                self.log("Leecher detected, %s is only sharing %s files in %s folders. Banned and Ignored", (user, num_files, num_folders))
            ...
            if not self.settings["suppress_all_messages"]:
                self.log("User %s has enough files and is not banned.", user)
    ...
    def upload_finished_notification(self, user, *_):
        ...
        # Only send message if sending messages to banned users is enabled
        if self.settings["send_message_to_banned"] and self.settings["message"]:
            ...
            if not self.settings["suppress_all_messages"]:
                self.log("Leecher %s doesn't share enough files. No message sent as 'send_message_to_banned' is disabled.", user)
