...
class Plugin(BasePlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.settings = {
            ...
            "bypass_share_limit_for_buddies": self.metasettings[
                "bypass_share_limit_for_buddies"
            ]["default"],
            "send_message_to_banned": self.metasettings["send_message_to_banned"][
                "default"
            ],
            "suppress_banned_user_logs": self.metasettings["suppress_banned_user_logs"][
                "default"
            ],
            "suppress_ignored_user_logs": self.metasettings[
                "suppress_ignored_user_logs"
            ]["default"],
            "suppress_ip_ban_logs": self.metasettings["suppress_ip_ban_logs"][
                "default"
            ],
            "suppress_request_logs": self.metasettings["suppress_request_logs"][
                "default"
            ],
            "suppress_all_messages": self.metasettings["suppress_all_messages"][
                "default"
            ],
            "suppress_meets_criteria_logs": self.metasettings[
                "suppress_meets_criteria_logs"
            ]["default"],
            "detected_leechers": [],
            "startup_delay": self.metasettings["startup_delay"][
                "default"
            ],  # Initialize delay from settings
        }
    ...
    def schedule_notification_suppression_reset(self):
        ...
        Timer(
            self.settings["startup_delay"], self.reset_notification_suppression
        ).start()
    ...
    def reset_notification_suppression(self):
        ...
        if not self.settings.get("suppress_all_messages", False):
            self.log(
                "Notification suppression lifted after %d seconds.",
    ...
    def log(self, message, *args):
        ...
        if not self.notifications_suppressed or not self.settings.get(
    ...
    def loaded_notification(self):
        ...
        if not self.settings.get("suppress_all_messages", False):
            self.log(
                "Users need at least %d files and %d folders.",
    ...
    def check_user(self, user, num_files, num_folders):
        ...
        if (
            user in self.previous_buddies
            and not self.probed_users.get(user) == "requesting_stats"
        ):
            if not self.settings["bypass_share_limit_for_buddies"]:
                if not self.notifications_suppressed and not self.settings.get(
                    "suppress_all_messages", False
                ):
                    self.log(
                        "Buddy %s is sharing %d files in %d folders. Skipping check.",
                        user,
                        num_files,
                        num_folders,
                    )
            ...
            if is_user_accepted:
                if not self.notifications_suppressed:
                    if user not in self.logged_scans:
                        if not self.settings.get("suppress_all_messages", False):
                            self.log(
                                "User %s is okay, sharing %d files in %d folders.",
                                user,
                                num_files,
                                num_folders,
                            )
                ...
                if not self.notifications_suppressed:
                    if user not in self.logged_scans:
                        if not self.settings.get("suppress_all_messages", False):
                            self.log(
                                "Buddy %s is sharing %d files in %d folders. Not complaining.",
                                user,
                                num_files,
                                num_folders,
                            )
    ...
    def upload_queued_notification(self, user):
    ...
    def upload_finished_notification(self, user, *_):
        ...
        self.ban_user(
            user, num_files=self.uploaded_files_count.get(user, 0), num_folders=0
        )
        if self.settings["ban_block_ip"]:
            self.block_ip(user)
        if not self.notifications_suppressed:
            ...
            if not self.notifications_suppressed:
                if not self.settings.get("suppress_banned_user_logs", False):
                    if username not in self.logged_scans:
                        log_message = (
                            "Banned Leecher %s - Sharing: %d files, %d folders"
                            % (username, num_files, num_folders)
                        )
